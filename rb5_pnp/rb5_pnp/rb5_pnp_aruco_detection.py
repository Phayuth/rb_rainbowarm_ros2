...
class ArUcoSimplePoseEstimator(Node):
    ...
    def __init__(self) -> None:
        ...
        self.imgcsub = self.create_subscription(
            Image,
        ...
        self.infcsub = self.create_subscription(
            CameraInfo,
        ...
        self.imgdpub = self.create_publisher(
            Image, "/detected_aruco", 2, callback_group=self.ccb
        )
        self.arcpub = self.create_publisher(
            PoseStamped, "/arc_pose", 2, callback_group=self.ccb
        )
    ...
    def pose_estimate(self, imageRaw, cameraMatrix, distCoeffs):
        ...
        imageUndst = cv2.undistort(
            imageRaw, cameraMatrix, distCoeffs, None, newcameramtx
        )
    ...
    def imgc_cb(self, msg: Image):
        ...
        if self.cameraMatrix is not None:
            imageUndst, retrvc, rettvc = self.pose_estimate(
                imgRGB, self.cameraMatrix, self.distCoeffs
            )
            if retrvc is not None:
                rM, _ = cv2.Rodrigues(retrvc)
                H = rbt.conv_rotmat_and_t_to_h(
                    rM,
                    rettvc.reshape(
                        3,
                    ),
            ...
            self.imgdpub.publish(
                self.cvbride.cv2_to_imgmsg(imgBGR, encoding="rgb8")
            )
