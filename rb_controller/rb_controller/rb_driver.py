...
def GetCurrentJoint():
    if not data_connect:
        current_joint_ = Joint(systemstat_global.jnt_ref[0], systemstat_global.jnt_ref[1], systemstat_global.jnt_ref[2], systemstat_global.jnt_ref[3], systemstat_global.jnt_ref[4], systemstat_global.jnt_ref[5])
...
def GetCurrentSplitedJoint():
    if not data_connect:
        current_joint_ = (systemstat_global.jnt_ref[0], systemstat_global.jnt_ref[1], systemstat_global.jnt_ref[2], systemstat_global.jnt_ref[3], systemstat_global.jnt_ref[4], systemstat_global.jnt_ref[5])
        return current_joint_
    else:
        print("'data socket isn't connect")
...
def GetCurrentTCP():
    if not data_connect:
        current_tcp_ = Point(systemstat_global.tcp_ref[0], systemstat_global.tcp_ref[1], systemstat_global.tcp_ref[2], systemstat_global.tcp_ref[3], systemstat_global.tcp_ref[4], systemstat_global.tcp_ref[5])
        return current_tcp_
    else:
        print("'data socket isn't connect")
...
def GetCurrentSplitedTCP():
    if not data_connect:
        current_tcp_ = (systemstat_global.tcp_ref[0], systemstat_global.tcp_ref[1], systemstat_global.tcp_ref[2], systemstat_global.tcp_ref[3], systemstat_global.tcp_ref[4], systemstat_global.tcp_ref[5])
        return current_tcp_
    else:
        print("'data socket isn't connect")
        current_tcp_ = ()
...
def ServoJ(j0, j1, j2, j3, j4, j5, t1, t2, gain, alpha):
    msg = f"move_servo_j(jnt[{j0}, {j1}, {j2}, {j3}, {j4}, {j5}], {t1}, {t2}, {gain}, {alpha})"
    return SendCOMMAND(msg, CMD_TYPE.MOVE)


def ServoL(x, y, z, rx, ry, rz, t1, t2, gain, alpha):
    msg = f"move_servo_l(pnt[{x}, {y}, {z}, {rx}, {ry}, {rz}], {t1}, {t2}, {gain}, {alpha})"
    return SendCOMMAND(msg, CMD_TYPE.MOVE)


def ServoT(jt0, jt1, jt2, jt3, jt4, jt5, t1, t2, mode):
    msg = f"move_servo_t(jnt[{jt0}, {jt1}, {jt2}, {jt3}, {jt4}, {jt5}], {t1}, {t2}, {mode})"
    return SendCOMMAND(msg, CMD_TYPE.MOVE)


def SpeedJ(jd0, jd1, jd2, jd3, jd4, jd5, t1, t2, gain, alpha):
    msg = f"move_speed_j(jnt[{jd0}, {jd1}, {jd2}, {jd3}, {jd4}, {jd5}], {t1}, {t2}, {gain}, {alpha})"
    return SendCOMMAND(msg, CMD_TYPE.MOVE)


def SpeedL(xd, yd, zd, rxd, ryd, rzd, t1, t2, gain, alpha):
    msg = f"move_speed_l(pnt[{xd}, {yd}, {zd}, {rxd}, {ryd}, {rzd}], {t1}, {t2}, {gain}, {alpha})"
    return SendCOMMAND(msg, CMD_TYPE.MOVE)


@dispatch(Point, float, float)
def MoveL(pnt, spd, acc):
    msg = "move_l(pnt[" + str(pnt.x) + "," + str(pnt.y) + "," + str(pnt.z) + "," + str(pnt.rx) + "," + str(pnt.ry) + "," + str(pnt.rz) + "], " + str(spd) + "," + str(acc) + ")"
    return SendCOMMAND(msg, CMD_TYPE.MOVE)


@dispatch(float, float, float, float, float, float, float, float)
def MoveL(x, y, z, rx, ry, rz, spd, acc):
    msg = "move_l(pnt[" + str(x) + "," + str(y) + "," + str(z) + "," + str(rx) + "," + str(ry) + "," + str(rz) + "], " + str(spd) + "," + str(acc) + ")"
    return SendCOMMAND(msg, CMD_TYPE.MOVE)


@dispatch(Joint, float, float)
def MoveJ(jnt, spd, acc):
    msg = "move_j(jnt[" + str(jnt.j0) + "," + str(jnt.j1) + "," + str(jnt.j2) + "," + str(jnt.j3) + "," + str(jnt.j4) + "," + str(jnt.j5) + "], " + str(spd) + "," + str(acc) + ")"
    return SendCOMMAND(msg, CMD_TYPE.MOVE)


@dispatch(float, float, float, float, float, float, float, float)
def MoveJ(j0, j1, j2, j3, j4, j5, spd, acc):
    msg = "move_j(jnt[" + str(j0) + "," + str(j1) + "," + str(j2) + "," + str(j3) + "," + str(j4) + "," + str(j5) + "], " + str(spd) + "," + str(acc) + ")"
    return SendCOMMAND(msg, CMD_TYPE.MOVE)


@dispatch(Point, float, float)
def MoveJL(pnt, spd, acc):
    msg = "move_jl(pnt[" + str(pnt.x) + "," + str(pnt.y) + "," + str(pnt.z) + "," + str(pnt.rx) + "," + str(pnt.ry) + "," + str(pnt.rz) + "], " + str(spd) + "," + str(acc) + ")"
    return SendCOMMAND(msg, CMD_TYPE.MOVE)


@dispatch(float, float, float, float, float, float, float, float)
def MoveJL(x, y, z, rx, ry, rz, spd, acc):
    msg = "move_jl(pnt[" + str(x) + "," + str(y) + "," + str(z) + "," + str(rx) + "," + str(ry) + "," + str(rz) + "], " + str(spd) + "," + str(acc) + ")"
    return SendCOMMAND(msg, CMD_TYPE.MOVE)
...
@dispatch(Joint)
def MoveJB_Add(jnt):
    msg = "move_jb_add(jnt[" + str(jnt.j0) + "," + str(jnt.j1) + "," + str(jnt.j2) + "," + str(jnt.j3) + "," + str(jnt.j4) + "," + str(jnt.j5) + "])"
    return SendCOMMAND(msg, CMD_TYPE.NONMOVE)


@dispatch(float, float, float, float, float, float)
def MoveJB_Add(j0, j1, j2, j3, j4, j5):
    msg = "move_jb_add(jnt[" + str(j0) + "," + str(j1) + "," + str(j2) + "," + str(j3) + "," + str(j4) + "," + str(j5) + "])"
    return SendCOMMAND(msg, CMD_TYPE.NONMOVE)
...
def MovePB_Add(pnt, spd, option, quantity):
    ...
    msg = "move_pb_add(pnt[" + str(pnt.x) + "," + str(pnt.y) + "," + str(pnt.z) + "," + str(pnt.rx) + "," + str(pnt.ry) + "," + str(pnt.rz) + "], " + str(spd) + "," + str(b_option) + "," + str(quantity) + ")"
...
def MovePB_Add(x, y, z, rx, ry, rz, spd, option, quantity):
    ...
    msg = "move_pb_add(pnt[" + str(x) + "," + str(y) + "," + str(z) + "," + str(rx) + "," + str(ry) + "," + str(rz) + "], " + str(spd) + "," + str(b_option) + "," + str(quantity) + ")"
    return SendCOMMAND(msg, CMD_TYPE.NONMOVE)
...
def MoveITPL_Add(pnt, spd):
    msg = "move_itpl_add(pnt[" + str(pnt.x) + "," + str(pnt.y) + "," + str(pnt.z) + "," + str(pnt.rx) + "," + str(pnt.ry) + "," + str(pnt.rz) + "], " + str(spd) + ")"
...
def MoveITPL_Add(x, y, z, rx, ry, rz, spd):
    msg = "move_itpl_add(pnt[" + str(x) + "," + str(y) + "," + str(z) + "," + str(rx) + "," + str(ry) + "," + str(rz) + "], " + str(spd) + ")"
...
def MoveCircle_ThreePoint(x1, y1, z1, rx1, ry1, rz1, x2, y2, z2, rx2, ry2, rz2, spd, acc, type):
    ...
    msg = "move_c_points(pnt[" + str(x1) + "," + str(y1) + "," + str(z1) + "," + str(rx1) + "," + str(ry1) + "," + str(rz1) + "], pnt[" + str(x2) + "," + str(y2) + "," + str(z2) + "," + str(rx2) + "," + str(ry2) + "," + str(rz2) + "], " + str(spd) + "," + str(acc) + "," + str(r_option) + ")"
...
def MoveCircle_Axis(pnt, axis, direction, angle, spd, acc, type):
    ...
    msg = "move_c_axis(pnt[" + str(pnt.x) + "," + str(pnt.y) + "," + str(pnt.z) + "," + str(pnt.rx) + "," + str(pnt.ry) + "," + str(pnt.rz) + "], " + str(a_option) + "," + str(angle) + "," + str(spd) + "," + str(acc) + "," + str(r_option) + ")"
...
def MoveCircle_Axis(x, y, z, rx, ry, rz, axis, direction, angle, spd, acc, type):
    ...
    msg = "move_c_axis(pnt[" + str(x) + "," + str(y) + "," + str(z) + "," + str(rx) + "," + str(ry) + "," + str(rz) + "], " + str(a_option) + "," + str(angle) + "," + str(spd) + "," + str(acc) + "," + str(r_option) + ")"
        ...
        if msg_recv[0] == 0x24:
            ...
            if size <= len(msg_recv) - 4:
                if msg_recv[3] == 3:
                    ...
                    result = struct.unpack("fffffffffffffffffffffffffffffffffffffffiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiffffffiiiififiiffffffiiiiiiiiiiiffiiiifiiiiiiiiiiiffffff", msg_recv_split)
...
if __name__ == "__main__":
    ...
    # speed slider
    spdslider = 1.0
    print(f"Robot Speed Slider is set to : {spdslider}")
    SetBaseSpeed(spdslider)

    # Mode Real or Simulation
    SetProgramMode(PG_MODE.REAL)  # set to real robot
